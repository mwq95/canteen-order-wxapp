...
def add_comments_to_file(file_path):
    ...
    # 为每个文件添加特定的注释
    commented_content = add_specific_comments(content, page_name, file_path)
...
def add_specific_comments(content, page_name, file_path):
    # 页面描述映射
    page_descriptions = {
    ...
    def add_data_comments(match):
        ...
        # 为每个数据字段添加注释
        for field, comment in data_fields.items():
            pattern = rf'({field}:)'
            replacement = rf'// {comment}\n    \1'
            data_content = re.sub(pattern, replacement, data_content)
        
        return f'data: {{\n{data_content}\n  }},'
    
    # 替换 data 对象
    content = re.sub(r'data:\s*\{([\s\S]*?)\s*\},', add_data_comments, content)
